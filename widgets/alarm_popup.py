...
class AlarmPopup(ModalView):
    """Попап для срабатывания будильника"""
    ...
    def on_snooze(self, button):
        ...
        if hasattr(app, 'alarm_clock') and app.alarm_clock:
            app.alarm_clock.snooze_alarm(5)  # 5 минут
    ...
    def on_dismiss_button(self, button):
        """Обработка кнопки отключить будильник"""
    ...
    def on_dismiss(self):
        """ИСПРАВЛЕНО: Метод вызывается Kivy при закрытии попапа"""
        # Этот метод вызывается Kivy без аргументов
        logger.info("AlarmPopup dismissed")
        
        # Останавливаем будильник при закрытии любым способом
        app = App.get_running_app()
        if hasattr(app, 'alarm_clock') and app.alarm_clock:
            app.alarm_clock.stop_alarm()
    ...
    def open_alarm(self):
        ...
        app = App.get_running_app()
        if hasattr(app, 'audio_service') and self.ringtone:
            try:
                import os
